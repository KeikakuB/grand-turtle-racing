...
namespace PM3D
{
  ULONG_PTR TextHolder::token = 0;

  TextHolder::TextHolder(int largeur, int hauteur, Font* pPolice)
  {
    TexWidth = largeur;
    TexHeight = hauteur;
    ...
    HRESULT hr = GameSingleton::Get().GetDispositif()->GetD3DDevice()->CreateTexture2D(&texDesc, &data, &pTexture);
    ...
    hr = GameSingleton::Get().GetDispositif()->GetD3DDevice()->CreateShaderResourceView(pTexture, &srvDesc, &pTextureView);
  ...
  void TextHolder::Close()
  {
    //TODO(Bill): find where we can call this
  ...
  void TextHolder::Ecrire(wstring s)
  {
    // Effacer
    pCharGraphics->Clear(Gdiplus::Color(0, 0, 0, 0));
    ...
    GameSingleton::Get().GetDispositif()->GetImmediateContext()->UpdateSubresource(pTexture, 0, 0, bmData.Scan0, TexWidth * 4, 0);

    pCharBitmap->UnlockBits(&bmData);
  }
  void TextHolder::ChangeColorBrush(Gdiplus::Color* couleur)
  {
    if ( !couleur )
    {
      pBlackBrush = new SolidBrush(Gdiplus::Color(255, 0, 0, 0));
    ...
    {
      pBlackBrush = new SolidBrush(*couleur);
    }
  }
}